...
class MapObj<Value>{
    ...
    static of<T>(obj?: Record<string, T>): MapObj<T>{
        return new MapObj(obj)
    }

    constructor(obj:Record<string,Value>){
        this.$value = obj
    }

    insert(key:string, val:Value): MapObj<Value>{
        const singleton = {} as Record<string, Value>
        singleton[key] = val
        return MapObj.of({...this.$value, ...singleton})
    }

    reduceWithKeys<A>(fn: (resultType:A, value:Value, key:string) => A, initialValue:A): A{
        return Object.keys(this.$value).reduce((prevVal:A, currKey:string) => fn(prevVal, this.$value[currKey], currKey), initialValue)
    }
    
    map<A>(fn:(value:Value)=> A):MapObj<A>{
        return this.reduceWithKeys( (mapObj:MapObj<A>, val:Value, key:string) => mapObj.insert(key, fn(val)), MapObj.of({} as Record<string, A>) )
    }

    lookupValue(key:string):Maybe<Value>{
        return Maybe.of(this.$value[key])
    }
}
